...
-- Apply custom colors based on selected theme
local function PushThemeColors()
  local count = 0
  if state.theme == 0 then
    -- Dark Theme (Default BG, Dark Green inputs/buttons/title)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBg(), 0x1E4D2BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBgActive(), 0x2E6D3BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBgCollapsed(), 0x1E4D2BFF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBg(), 0x1E4D2BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBgHovered(), 0x2E6D3BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBgActive(), 0x3E8D4BFF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Button(), 0x1E4D2BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_ButtonHovered(), 0x2E6D3BFF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_ButtonActive(), 0x3E8D4BFF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_CheckMark(), 0x4CAF50FF)
    count = 10
  else
    -- Light Theme (Light Gray BG, Dark Gray inputs/buttons/title)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBg(), 0x808080FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBgActive(), 0x909090FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_TitleBgCollapsed(), 0x808080FF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_WindowBg(), 0xE0E0E0FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), 0x202020FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_ChildBg(), 0xD0D0D0FF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBg(), 0x808080FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBgHovered(), 0x909090FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_FrameBgActive(), 0xA0A0A0FF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Button(), 0x808080FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_ButtonHovered(), 0x909090FF)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_ButtonActive(), 0xA0A0A0FF)
    
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_CheckMark(), 0x202020FF)
    count = 13
  end
  return count
end
...
local function DrawWindow()
  ...
  local color_count = PushThemeColors()
  ...
  if visible then
    ...
    if reaper.ImGui_RadioButton(ctx, "Dark", state.theme == 0) then
      state.theme = 0
    end
    reaper.ImGui_SameLine(ctx)
    if reaper.ImGui_RadioButton(ctx, "Light", state.theme == 1) then
      state.theme = 1
    end
    ...
    reaper.ImGui_TextWrapped(ctx, "Choose which regions to affect. If none are checked, ALL visible regions will be affected.")
    reaper.ImGui_Spacing(ctx)
  ...
  if reaper.ImGui_Button(ctx, "Apply Changes", avail_width, 30) then
    ...
    local any_checked = false
    for _, r in ipairs(state.visible_regions) do
      if state.selected_regions[r.index] then
        any_checked = true
        break
      end
